"""
Fine-tune a sequence-classification model on the EstateIQ transcript
sentiment data produced by prepare_sentiment_data.py.

Usage:
    python app/nlp/train_sentiment.py \
        --model distilbert-base-multilingual-cased \
        --data-dir data/sentiment \
        --out-dir models_artifacts/sentiment \
        --epochs 6
"""
...
# Reuse the in-memory best-checkpoint callback from the NER trainer -- disk
# on this machine is tight, so we avoid writing a full model copy to disk
# after every epoch and only save once, at the end, with the best weights.
...
def main():
    ...
    ap.add_argument("--out-dir", default="models_artifacts/sentiment")
    ...
    training_args = TrainingArguments(
        ...
        eval_strategy="epoch",
        save_strategy="no",  # see BestModelInMemoryCallback note above
